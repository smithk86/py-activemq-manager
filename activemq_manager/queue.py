...
class Queue:
    ...
    async def attributes(self, attribute=None) -> Dict[str, Any]:
        return await self._client.dict_request(
            "read",
            f"org.apache.activemq:type=Broker,brokerName={self.broker.name},destinationType=Queue,destinationName={self.name}",
            attribute=attribute,
        )
    ...
    async def purge(self) -> None:
        await self._client.request(
            "exec",
            f"org.apache.activemq:brokerName={self.broker.name},type=Broker,destinationType=Queue,destinationName={self.name}",
            operation="purge",
        )
    ...
    async def delete(self) -> None:
        await self._client.request(
            "exec",
            f"org.apache.activemq:type=Broker,brokerName={self.broker.name}",
    ...
    async def messages(self, selector=None) -> List[Message]:
        if selector:
            message_table = await self._client.dict_request(
                "exec",
                f"org.apache.activemq:brokerName={self.broker.name},type=Broker,destinationType=Queue,destinationName={self.name}",
                operation="browseAsTable(java.lang.String)",
                arguments=[selector],
            )
        else:
            message_table = await self._client.dict_request(
        ...
        if self.size > len(message_table):
            logger.warning(
                f"queue size is greater than the returned number of messages [qsize={self.size}, message={len(message_table)}]; use a selector to reduce the total number of messages"
            )
