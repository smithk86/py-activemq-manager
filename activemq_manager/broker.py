...
class Broker:
    ...
    def __init__(
        self, client: Client, name: str = "localhost", workers: int = 10
    ) -> None:
        self._client = client
        self.name = name
    ...
    async def attributes(self) -> Dict[str, Any]:
        return await self._client.dict_request(
            "read", f"org.apache.activemq:type=Broker,brokerName={self.name}"
        )
    ...
    async def queues(self) -> AsyncGenerator:
        ...
        _queue_names = list()
        for object_name in await self._client.list_request(
            "search",
            f"org.apache.activemq:type=Broker,brokerName={self.name},destinationType=Queue,destinationName=*",
        ):
    ...
    async def queue(self, name):
        queue_objects = await self._client.list_request(
            "search",
            f"org.apache.activemq:type=Broker,brokerName={self.name},destinationType=Queue,destinationName={name}",
        )
    ...
    async def _jobs(
        ...
        return await self._client.dict_request(
            ...
            arguments=[start.strftime(Broker.dtformat), end.strftime(Broker.dtformat)],
        )
    ...
    async def _connections(self) -> AsyncGenerator:
        for connection_type in (await self.attribute("TransportConnectors")).keys():
            for object_name in await self._client.list_request(
                "search",
                f"org.apache.activemq:type=Broker,brokerName={self.name},connector=clientConnectors,connectorName={connection_type},connectionViewType=remoteAddress,connectionName=*",
            ):
            ...
            if "connectionName" in _parsed_object_name:
                _connections.append(
                    self._connection_class(
                        self, _parsed_object_name["connectionName"], connection_type
                    )
                )
            else:
                raise ActivemqManagerError(
                    f"connectionName property not found in {_parsed_object_name}"
                )
